...
mod tests {
        ...
        *resolved_hint_label.lock() = Some(lsp::InlayHintLabel::LabelParts(vec![
        ...
        cx.update_editor(|editor, window, cx| {
            editor.update_inlay_link_and_hover_points(
                ...
                Some(new_type_hint_part_hover_position),
                None,
                true,
                false,
                window,
        ...
        cx.update_editor(|editor, window, cx| {
            editor.update_inlay_link_and_hover_points(
                &editor.snapshot(window, cx),
                Some(struct_hint_part_hover_position),
                None,
                true,
                false,
                window,
    ...
    async fn test_hover_pending_hint_survives_previous_popover_hiding(
                ...
                initializer: Some(Box::new({
                    ...
                    move |server| {
                        server.set_request_handler::<lsp::request::InlayHintRequest, _, _>(
                            move |_, _| async move {
                                Ok(Some(
                                    [(17, "a", false), (28, "b", true), (39, "c", true)]
                                        .into_iter()
                                        .map(|(column, name, resolve_tooltip)| lsp::InlayHint {
                                            ...
                                            tooltip: (!resolve_tooltip).then(|| {
                                                lsp::InlayHintTooltip::String(format!(
                                                    "tooltip {name}"
                                                ))
                                            }),
                                            padding_left: None,
        ...
        .await;
        cx.set_state("fn main() { let aˇ = 1; let b = 2; let c = 3; }\n");
        cx.run_until_parked();
        cx.update_editor(|editor, _, cx| {
            assert_eq!(visible_hint_labels(editor, cx), vec![": a", ": b", ": c"]);
        });
