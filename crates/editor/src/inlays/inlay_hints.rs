...
pub(super) struct PendingHint {
    buffer_id: BufferId,
    pub(super) hint_id: InlayId,
    resolve: Option<Shared<Task<Option<InlayHint>>>>,
}

impl PendingHintTextEdits {
    fn remove_hints(&mut self, mut remove: impl FnMut(&PendingHint) -> bool) {
        let hints_before = self.hints.len();
        self.hints.retain(|hint| !remove(hint));
...
impl LspInlayHintData {
    ...
    pub fn clear_for_buffers(
        ...
        self.remove_pending_text_edits(|hint| buffer_ids.contains(&hint.buffer_id));
    ...
    pub(super) fn remove_pending_text_edits(&mut self, remove: impl FnMut(&PendingHint) -> bool) {
        if let Some(pending_text_edits) = &mut self.pending_text_edits {
            pending_text_edits.remove_hints(remove);
...
impl Editor {
    ...
    fn apply_pending_inlay_hint_text_edits(
        ...
        {
            if !inlay_hints.added_hints.contains_key(&hint_id) {
                continue;
            }
