...
mod tests {
    ...
    use gpui::{
        Modifiers, MouseButton, MouseDownEvent, MouseMoveEvent, MousePressureEvent, MouseUpEvent,
        PlatformInput, PressureStage,
    };
    use indoc::indoc;
    ...
    async fn test_accept_inlay_hint(cx: &mut gpui::TestAppContext) {
        ...
        let accepted_state = "use std::primitive::u32;\nfn main() { let value: u32ˇ = 1; }\n";
        ...
        *hints.lock() = Some(vec![type_hint(21, "u32", HintEdits::None)]);
        ...
        for (edits, expected_state) in [
            (HintEdits::OnResolve, accepted_state),
            (HintEdits::None, source),
        ] {
            *hints.lock() = Some(vec![type_hint(21, "u32", edits)]);
            cx.set_state(source);
            cx.run_until_parked();
        ...
        *hints.lock() = Some(vec![
            type_hint(17, "u32", HintEdits::OnResolve),
            parameter_hint(27, "b", HintEdits::InResponse),
        ]);
...
"
    ...
    fn parameter_hint(column: u32, name: &str, edits: HintEdits) -> lsp::InlayHint {
        let position = lsp::Position::new(0, column);
        let label = format!("{name}: ");
        lsp::InlayHint {
            position,
            text_edits: (edits == HintEdits::InResponse).then(|| {
                vec![lsp::TextEdit {
                    range: lsp::Range::new(position, position),
